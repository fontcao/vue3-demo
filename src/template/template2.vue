  ...
  <div class="template2">
    <div class="resume-container" :class="layout">
      ...
      <div :class="layout === 'horizontal' ? 'right-section' : 'resume-main'">
        ...
        <Education v-if="layout === 'vertical'" :data="resumeData.educational" @click="openEditDrawer('education')" />
        ...
        <SelfEvaluation v-if="layout === 'vertical'" :data="resumeData" @click="openEditDrawer('selfevaluation')" />
      </div>
    </div>
...
const openEditDrawer = (section) => {
  ...
  if (section === 'personal') {
    ...
    // 工作经验是数组
    Object.assign(editFormData, resumeData.workexperience)
  } else if (section === 'project') {
    // 项目经历是数组
    Object.assign(editFormData, resumeData.projectexperience)
  } else if (section === 'social') {
    // 社会实践是数组
    Object.assign(editFormData, resumeData.socialpractice)
  } else if (section === 'certificate') {
    // 证书是数组
    Object.assign(editFormData, resumeData.certificaterewards)
  } else if (section === 'reward') {
    // 奖励是数组
    Object.assign(editFormData, resumeData.archiveRewards)
  } else if (section === 'skill') {
    // 技能是数组
    Object.assign(editFormData, resumeData.skills)
  } else if (section === 'achievement') {
    // 成果是数组
    Object.assign(editFormData, resumeData.archievementdisplay)
...
const saveEdit = (section, data) => {
  ...
  } else if (section === 'self' || section === 'selfevaluation') {
